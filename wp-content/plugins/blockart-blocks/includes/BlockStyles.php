...
class BlockStyles {
	...
	public function __construct( array &$blocks, $id = null, bool $force_generate = false ) {
		$this->setting        = blockart_get_setting();
	...
	public function enqueue( $version = false ) {
		...
		if ( ! blockart_is_preview() ) {
			$inline = ! ( $this->setting->get( 'asset-generation.external-file', false ) && file_exists( BLOCKART_UPLOAD_DIR . "/$this->filename" ) );
		}

		if ( ! $inline ) {
			wp_enqueue_style(
				"blockart-blocks-css-$this->id",
				BLOCKART_UPLOAD_DIR_URL . "/$this->filename",
		...
		wp_add_inline_style( 'blockart-blocks', $this->styles );
		unset( $this->styles );
	}
	...
	public function enqueue_fonts( $additional_fonts = array() ) {
		...
		if ( $this->setting->get( 'performance.local-google-fonts', false ) ) {
			...
			$google_fonts_url = blockart_get_webfont_url( $google_fonts_url, 'woff2', $preload );
		}
	...
	private function dimension( $value, $styles_def, $attrs, $attrs_def, $wrapper_class ) {
		...
		foreach ( $styles_def as $style_def ) {
			...
			if ( blockart_array_has_any( $value, self::DEVICES ) ) {
				foreach ( self::DEVICES as $device ) {
	...
	private function get_attribute_def( $block_namespace ) {
		...
		if ( $attribute_def && 'blockart/section' === $block_namespace ) {
			$attribute_def['width']['default']['desktop']['value'] = $this->setting->get( 'editor.section-width', 1170 );
		}
	...
	private function create_style_file() {
		...
		if (
			! $filesystem->is_dir( BLOCKART_UPLOAD_DIR ) &&
		...
		if ( $files ) {
			foreach ( $files as $file ) {
				if ( false !== strpos( $file['name'], "ba-style-$this->id-" ) ) {
					$filesystem->delete( BLOCKART_UPLOAD_DIR . '/' . $file['name'] );
				}
			}
		}

		$this->filename = "ba-style-$this->id-" . time() . '.css';

		return $filesystem->touch( BLOCKART_UPLOAD_DIR . '/' . $this->filename );
	}


	/**
	 * This function writes the contents of a file to a specified directory using the BlockArt filesystem.
	 */
	private function write() {
		$filesystem = blockart_get_filesystem();
		if ( $filesystem ) {
			$filesystem->put_contents( BLOCKART_UPLOAD_DIR . '/' . $this->filename, $this->styles );
		}
	}
