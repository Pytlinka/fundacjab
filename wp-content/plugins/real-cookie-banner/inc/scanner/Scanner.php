...
class Scanner
{
    ...
    {
        return isset($_GET[self::QUERY_ARG_TOKEN]) && \current_user_can(RealQueueCore::MINIMUM_QUERY_CAPABILITY) ? \true : $enabled;
    }
    /**
    ...
    public function resolve_blockables($blockables, $headlessContentBlocker)
    {
        ...
        foreach ($blockers as $blocker) {
            ...
            // Currently, our `FalsePositivesProcessor` does not support rule groups
            //$ruleGroups = $otherMeta['ruleGroups'] ?? [];
            if (\count($rules) > 0) {
                $blockables[] = new ScannableBlockable($headlessContentBlocker, $identifier, $extendsIdentifier, $rules);
            }
        }
        return $blockables;
    }
