...
class ConfigPage
{
    ...
    public function admin_notices_services_with_empty_privacy_policy()
    {
        ...
        foreach ($servicesWithoutPrivacyPolicy as $serviceId) {
            ...
            $liOutput[] = \sprintf('<li data-id="%d">%s &bull; <a href="%s" target="%s">%s</a></li>', $serviceId, $service->post_title, \esc_attr($url . '#/cookies/' . $cookieGroup->term_id . '/edit/' . $serviceId), $isConfigPage ? '_top' : '_blank', \__('Set privacy policy URL', RCB_TD));
        }
        if (\count($liOutput) > 0) {
            echo \sprintf('<div class="notice notice-warning" style="%s" id="rcb-services-with-empty-privacy-policy-notice"><p>%s</p><ul>%s</ul></div>', Core::getInstance()->getRpmInitiator()->getPluginUpdater()->getCurrentBlogLicense()->getActivation()->hasInteractedWithFormOnce() ? '' : 'display:none;', \__('There are no privacy policies with further information linked for the following services in your cookie banner. We now consider these to be mandatory in order to comply with the information obligations under the GDPR. Please provide a privacy policy for each service!', RCB_TD), \join('', $liOutput));
        }
    }
    /**
