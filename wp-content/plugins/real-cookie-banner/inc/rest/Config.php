...
class Config
{
    ...
    public function routeGetRevisionCurrent()
    {
        ...
        return new WP_REST_Response(\array_merge(
            // Expensive calculations, which should not be part of `getCurrent`
            ['needs_retrigger' => Revision::getInstance()->needsRetrigger($current), 'services_data_processing_in_unsafe_countries_notice_html' => Core::getInstance()->getNotices()->servicesDataProcessingInUnsafeCountriesNoticeHtml()],
            $current
        ));
    }
    /**
    ...
    {
        if ($request->get_route() === '/wp/v2/settings' && $request->get_method() === 'PATCH' && isset($response->data) && \is_array($response->data)) {
